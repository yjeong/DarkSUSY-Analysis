...
void DarkSUSY_FiducialRegion(){
	...
	bool isDiMuonHLTFired, is4GenMu8, is1SelMu17 , is2SelMu8, is3SelMu8;
	int event;
	...
	for(int i = 0; i < Sample_Num; i++){
		...
		tree[i]->SetBranchAddress("is1SelMu17",&is1SelMu17);
		tree[i]->SetBranchAddress("is2SelMu8",&is2SelMu8);
		tree[i]->SetBranchAddress("is3SelMu8",&is3SelMu8);
...
if(nSam==15){//mN1_60_mGammaD_35_cT_0p05
	l_8[nVar]->AddEntry(histo[nVar][nSam],Legend_name[nSam],"l");
	histo[nVar][nSam]->Draw("same");
	l_8[nVar]->Draw();
	//canv_[nVar]->SaveAs(Save_dir+"_"+Sample_name[nSam]+"_"+Variable[nVar]+".png");
...
const int ndeltaR = 1;
  float dR_cut[ndeltaR] = {0.5};
  TString dR_txt[ndeltaR] = {"dR<0.5"};//-*/
